package com.cg.hkrbudgeting.general.logic.base;
 ...
 * {@link javax.inject.Named} and {@link com.cg.hkrbudgeting.general.logic.api.UseCase}.
=======
 * Abstract base class for any <em>use case</em> in this application. Actual implementations need to
 * be annotated with {@link javax.inject.Named} and
 * {@link com.cg.hkrbudgeting.general.logic.api.UseCase}.
 *
>>>>>>> HEAD~0
 */
public abstract class AbstractUc extends AbstractBeanMapperSupport {
    ...
    /**
     * The limit for {@link net.sf.mmm.util.search.base.AbstractSearchCriteria#getMaximumHitCount()
     * maximum hit count} for UI requests.
     */
    ...
    /**
     * Maps a {@link PaginatedListTo paginated list} of persistent entities to a
     * {@link PaginatedListTo paginated list} of transfer objects.
     *
     ...
     * {@link Collection} using their {@link GenericEntity#getId() ID} as key. All
     * {@link GenericEntity entities} without an {@link GenericEntity#getId() ID} ({@code null})
     * will be ignored.
     *
     ...
     * @return a {@link Map} mapping from {@link GenericEntity#getId() ID} to {@link GenericEntity
     *         entity}.
     */
    ...
    /**
     * Determines the {@link GenericEntity entities} to delete if <code>currentList</code> is the
     * current list from the persistence and <code>listToSave</code> is the new list that shall be
     * saved. In other words this method selects the {@link GenericEntity entities} from
     * <code>currentList</code> that are not contained in <code>listToSave</code>.
     *
     * @param <ID>            is the generic type of the {@link GenericEntity#getId() ID}.
     * @param <E>             is the generic type of the {@link GenericEntity entity}.
     * @param currentEntities is the {@link Collection} of the {@link GenericEntity entities}
     *        currently persisted. We assume that all objects in this list have an
     *        {@link GenericEntity#getId() ID} value (that is not {@code null}).
     * @param entitiesToSave is the {@link Collection} that contains the {@link GenericEntity
     *        entities} that shall be saved. It may contain {@link GenericEntity entities} that have
     *        no {@link GenericEntity#getId() ID} that shall be newly created.
