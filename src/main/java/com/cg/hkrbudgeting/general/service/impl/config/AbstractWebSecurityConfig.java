...
/**
 * This type serves as a base class for extensions of the {@code WebSecurityConfigurerAdapter} and
 * provides a default configuration. <br/>
 * Security configuration is based on {@link WebSecurityConfigurerAdapter}. This configuration is by
...
public abstract class AbstractWebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    /**
     * Create a simple filter that allows logout on a REST Url /services/rest/logout and returns a
     * simple HTTP status 200 ok.
     *
     * @return the filter.
     */
    protected Filter getSimpleRestLogoutFilter() {
        ...
        LogoutFilter logoutFilter =
                new LogoutFilter(new LogoutSuccessHandlerReturningOkHttpStatusCode(),
    ...
    /**
     * Create a simple authentication filter for REST logins that reads user-credentials from a
     * json-parameter and returns status 200 instead of redirect after login.
     *
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication().withUser("waiter").password("waiter").roles("Waiter").and().withUser("cook")
                .password("cook").roles("Cook").and().withUser("barkeeper").password("barkeeper").roles("Barkeeper")
                .and()
