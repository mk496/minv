...
import com.cg.mobinv.reportengine.common.api.to.MediaResource;
...
public class ODataProcessor extends ODataSingleProcessor {
    ...
    @Autowired
    private ODataManagingService dataSource;
    ...
    public ODataResponse readEntitySet(final GetEntitySetUriInfo uriInfo, final String contentType)
        ...
        final EntityProviderWriteProperties feedProperties = EntityProviderWriteProperties
                .serviceRoot(context.getPathInfo().getServiceRoot()).inlineCountType(inlineCountType).inlineCount(count)
    ...
    public ODataResponse deleteEntity(final DeleteUriInfo uriInfo, final String contentType) throws ODataException {
        ...
        this.dataSource.deleteData(uriInfo.getStartEntitySet(), mapKey(uriInfo.getKeyPredicates()));
    ...
    public ODataResponse createEntity(final PostUriInfo uriInfo, final InputStream content,
        ...
        if (entityType.hasStream()) {
        	data = this.dataSource.createData(entitySet, data);
        	data = this.dataSource.writeBinaryData(entitySet, data,
        ...
        if (!navigationSegments.isEmpty()) {
            ...
            this.dataSource.writeRelation(previousEntitySet, sourceData, entitySet,
    ...
    public ODataResponse updateEntity(final PutMergePatchUriInfo uriInfo, final InputStream content,
        ...
        data = this.dataSource.updateData(entitySet, data);
        createInlinedEntities(entitySet, data, entryValues);
        expandSelectTree = entryValues.getExpandSelectTree();
    ...
    public ODataResponse deleteEntityLink(final DeleteUriInfo uriInfo, final String contentType) throws ODataException {
        ...
        final Object targetData = this.dataSource.readRelatedData(entitySet, sourceData, targetEntitySet, keys);
    ...
    public ODataResponse readEntityMedia(final GetMediaResourceUriInfo uriInfo, final String contentType)
        ...
        final EdmEntitySet entitySet = uriInfo.getTargetEntitySet();
        final BinaryData binaryData = this.dataSource.readBinaryData(entitySet, data);
        if (binaryData == null) {
            throw new ODataNotFoundException(ODataNotFoundException.ENTITY);
        }

        final String mimeType =
                binaryData.getMimeType() == null ? HttpContentType.APPLICATION_OCTET_STREAM : binaryData.getMimeType();
    ...
    public ODataResponse deleteEntityMedia(final DeleteUriInfo uriInfo, final String contentType)
        ...
        this.dataSource.writeBinaryData(uriInfo.getTargetEntitySet(), data, new BinaryData(null, null));
    ...
    public ODataResponse updateEntityMedia(final PutMergePatchUriInfo uriInfo, final InputStream content,
        ...
        final EdmEntitySet entitySet = uriInfo.getTargetEntitySet();
        this.dataSource.writeBinaryData(entitySet, data, new BinaryData(value, requestContentType));
    ...
    public ODataResponse executeFunctionImport(final GetFunctionImportUriInfo uriInfo, final String contentType)
        ...
        final Object data =
                this.dataSource
                        .readData(functionImport, mapFunctionParameters(uriInfo.getFunctionImportParameters()), null);

        if (data == null) {
            throw new ODataNotFoundException(ODataHttpException.COMMON);
        }

        if (data instanceof MediaResource) {
            MediaResource media = (MediaResource) data;
            return ODataResponse.fromResponse(EntityProvider.writeBinary(media.getMimeType(), media.getData())).build();
        }
    ...
    private Object retrieveData(final EdmEntitySet startEntitySet, final List<KeyPredicate> keyPredicates,
        ...
        try {
            data = functionImport == null
                    ?
                    keys.isEmpty() ?
                            this.dataSource.readData(startEntitySet) :
                            this.dataSource.readData(startEntitySet, keys)
                    :
                    this.dataSource.readData(functionImport, functionImportParameters, keys);

            EdmEntitySet currentEntitySet = functionImport == null ? startEntitySet : functionImport.getEntitySet();
            for (NavigationSegment navigationSegment : navigationSegments) {
                data = this.dataSource.readRelatedData(currentEntitySet, data, navigationSegment.getEntitySet(),
    ...
    private class WriteCallback implements OnWriteEntryContent, OnWriteFeedContent {
        ...
        private Object readRelatedData(final WriteCallbackContext context) throws ODataException {
            ...
            return ODataProcessor.this.dataSource.readRelatedData(entitySet,
                    this.data instanceof List
                            ?
                            readEntryData((List<?>) this.data, entitySet.getEntityType(),
            ...
            for (final T entryData : data) {
                ...
                for (final EdmProperty keyProperty : entityType.getKeyProperties()) {
                    if (!ODataProcessor.this.valueAccess.getPropertyValue(entryData, keyProperty)
                            .equals(key.get(keyProperty.getName()))) {
                        found = false;
                        break;
                    }
                }
    ...
    private Map<String, Object> parseLinkUri(final EdmEntitySet targetEntitySet, final String uriString)
        ...
        try {
            key =
                    UriParser.getKeyPredicatesFromEntityLink(targetEntitySet, uriString,
            ...
            if (relatedValue == null) {
                ...
                if (relatedValue instanceof ODataFeed) {
                    ...
                    for (final ODataEntry relatedValues : relatedValueList) {
                        ...
                        this.dataSource.writeRelation(entitySet, data, relatedEntitySet,
            ...
            case MEMBER:
                ...
                while (currentExpression != null) {
                    ...
                    currentExpression = currentExpression.getKind() == ExpressionKind.MEMBER
                            ? ((MemberExpression) currentExpression).getPath() : null;
                }
