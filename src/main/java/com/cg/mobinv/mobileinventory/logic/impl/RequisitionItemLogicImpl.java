...
public class RequisitionItemLogicImpl implements RequisitionItemLogic {
	...
	public List<RequisitionItemTo> readAll() {
		...
		List<RequisitionItemTo> resultMapped = queryResult.stream().map(x -> mapToTransferObject(x))
	...
	public RequisitionItemTo create(RequisitionItemTo itemTo) {
		...
		RequisitionItemEntity newItemEntity = mapToEntity(itemTo);
		this.itemRepository.save(newItemEntity);
		return mapToTransferObject(newItemEntity);
